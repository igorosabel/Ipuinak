...
import { CharacterInterface, CharactersResult } from '@interfaces/character.interfaces';
import { StatusIdResult, StatusResult } from '@interfaces/interfaces';
import { TaleInterface, TaleResult, TalesResult } from '@interfaces/tale.interfaces';
...
export default class ApiService {
  ...
  getTales(): Observable<Tale[]> {
    return this.http.post<TalesResult>(`${environment.apiUrl}/get-tales`, {}).pipe(
  ...
  saveTale(tale: TaleInterface): Observable<StatusIdResult> {
    return this.http.post<StatusIdResult>(`${environment.apiUrl}/save-tale`, tale);
  }

  getTale(id: number): Observable<Tale> {
    return this.http.post<TaleResult>(`${environment.apiUrl}/get-tale`, { id }).pipe(
  ...
  saveCharacter(character: CharacterInterface): Observable<StatusResult> {
    return this.http.post<StatusResult>(`${environment.apiUrl}/save-character`, character);
  }
